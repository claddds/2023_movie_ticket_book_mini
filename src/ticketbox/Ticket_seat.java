...
import movie_server.TicketCart_VO;
...
public class Ticket_seat extends JPanel {
	...
	// Icon ;
	String moviename,theater, adultCount, childCount, date, time, amount, room, seat;
	...
	public Ticket_seat(Sign_in signin) {
		...
		btnNewButton_1_1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				 ...
				 //성인과 아동 인원의 합
				 total = (aCount*10000)+(cCount*5000);
				 int count2 = map.su2;
				 //체크박스의 개수. 
				 ...
				 if (count2 > totalSelectedCount) {
					 ...
					 int res = JOptionPane.showOptionDialog(getParent(), "선택하신 좌석으로 예약할까요?", "좌석확인",
						...
						if (res == 0) {
							// 실제저장하는 코드
							moviename = sign_in.tb_pay.lblNewLabel_10.getText();	// 영화 이름 겟
							...
							// 결제를 위해 ticket_cart DB를 만들어서 카트에다가 담아둘것 결제는 아직 ㄴㄴ
							//TicketCart_VO tc_vo = new TicketCart_VO();
							MobileTicket_VO mt_vo = new MobileTicket_VO();
							TicketCart_VO tcvo = new TicketCart_VO();
							
							mt_vo.setMovie_name(moviename);
							mt_vo.setTheater_id(theater);
							mt_vo.setMovie_date(date);
							mt_vo.setStart_time(time);
							mt_vo.setTheater_seat(seat);
							
							int result_ticket = DAO.ticket_hj(mt_vo);
							System.out.println("티켓삽입 결과값"+result_ticket);
							
							tcvo.setTicket_num(mt_vo.getTicket_num());
							tcvo.setAmount(total);
						} else if (res == 1) {
							return;
						}
