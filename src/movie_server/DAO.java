...
public class DAO {
	...
	// 혜지 - Ticket_cart테이블에 결제 전 티켓정보 삽입
		public static int ticket_cart(TicketCart_VO vo) throws Exception {
			System.out.println("다오 도착");
			int result = getSession().insert("ticketcartinsert",vo);
			ss.commit();
			System.out.println("다오 값: "+result);
			return result;
		}
		
		//혜지 - ticketDB에 바로 삽입
		public static int ticket_hj(MobileTicket_VO vo) {
			System.out.println("ticket_hj다오 도착");
			int result = getSession().insert("ticketinsert_hj",vo);
			ss.commit();
			System.out.println("다오 값: "+result);
			return result;
		}
	...
	// 혜지
		// 결제금액 가져오기
		public static int paymoneyinsert() {
			int result = getSession().selectOne("paymoneyinsert");
			return result;
		}
